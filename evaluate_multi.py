...
def parse_args(args=None, namespace=None):
    ...
    parser.add_argument('--exp-setting', help='exp setting[digits, office, imageclef, visda]', default='office',
                        type=str)
    parser.add_argument('--model-path', help='model path for evaluation', default='/root/autodl-tmp/output/office_resnet50dsbn_SGD_lr=0.006/best_resnet50dsbn+None+i0_amazon2webcam.pth', type=str)
    parser.add_argument('--args-path', help='args path for evaluation. This code automatically ' +
                                            '"search base_dir/args_dict.pth" of model-path', default='/root/autodl-tmp/output/office_resnet50dsbn_SGD_lr=0.006/args_dict.pth', type=str)
    ...
    parser.add_argument('--save-results', help='whether to save the results', action='store_true',default=True)
...
def main():
    ...
    model = get_model(args.model_name, args.num_classes, args.in_features, pretrained=False, whitening=args_dict['whitening'], group_size=args_dict['group_size'], iter=args_dict['iter'], whitening_pos = args_dict['whitening_pos'], pre=args_dict['pre'], predir=args_dict['predir'])
    ...
    for d_idx, dataset in enumerate(target_test_datasets):
        ...
        with torch.no_grad():
            for i, (x_val, y_val) in enumerate(dataloader):
                ...
                if args.dsbn:
                    pred_val, f_val = model(x_val, torch.ones_like(y_val), with_ft=True)
                else:
