...
def parse_args(args=None, namespace=None):
    ...
    parser.add_argument('--model-name',
                        help="model name ['resnet50dsbw',  'resnet50', 'resnet50dsbn', 'resnet101', 'resnet101dsbn']",
                        default='resnet50dsbw', type=str)
    ...
    parser.add_argument('--save-dir', help='directory to save models', default='/root/autodl-tmp/output/office_resnet50dsbw_IN_all_last_t5_a2w_pretrain_lr=0.01', type=str)
    parser.add_argument('-m', help='描述当前实验信息', default='')
    parser.add_argument('--base-dir', help='输出文件根目录', default=r'/root/autodl-tmp/output/', type=str)
    
    # whitening options
    parser.add_argument('--whitening', help="the way of whitening [BW, GW, None]",default="None", type=str)
    parser.add_argument('--whitening-pos', help="posization of whitening [l0, last, all, l0+last]",default="last", type=str)
    parser.add_argument('--iter', help="iteration", default=5, type=int)
    parser.add_argument('--group-size', help="group_size for BW or num_groups for GW", default=32, type=int)
    parser.add_argument('--pre', help="whether use GW/BW pretrain? Pretrain/None", default="None", type=str)
    parser.add_argument('--predir', help="file of GW/BW pretrain", default="None", type=str)
    
    # model options
    parser.add_argument('--num-classes', help='number of classes', default=31, type=int)
    parser.add_argument('--source-datasets', help='source training dataset', default=['amazon'],
                        nargs='+')
    ...
    # machine options
    parser.add_argument('--num-workers', help='number of worker to load data', default=4, type=int)
    ...
    parser.add_argument('--warmup-learning-rate', '-wlr', help='warmup learning rate', default=0.5, type=float)
    parser.add_argument('--warmup-step', type=int, default=0, help='warm-up iterations')
    parser.add_argument('--optimizer', help='[Adam/SGD]', default='SGD', type=str)
    parser.add_argument('--learning-rate', '-lr', dest='learning_rate', help='learning_rate', default=0.01, type=float)
    ...
    parser.add_argument('--adv-loss', help='add domain loss', action='store_true', default=True)
    parser.add_argument('--domain-loss-adjust-factor', help='domain loss factor', default=0.1, type=float)
    parser.add_argument('--sm-loss', help='add moving semantic loss', action='store_true', default=True)
    ...
    parser.add_argument('--save-interval',
                        ...
                        default=500,
                        type=int)
    parser.add_argument('--print-console', help='activate console display', action='store_true', default=True)
...
def main():
    ...
    args.dsbn = True if 'dsb' in args.model_name else False  # set dsbn
    ...
    des = [] # office_resnetdsbw_BW_l0_5_32_amazon_webcam_SGD_0.01_pretrain_40
    des.append(str(args.exp_setting))
    des.append(str(args.model_name))
    des.append(str(args.whitening))
    des.append(str(args.whitening_pos))
    des.append(str(args.iter))
    des.append(str(args.group_size))
    des.append(str(args.source_datasets[0]))
    des.append(str(args.target_datasets[0]))
    des.append(str(args.optimizer))
    des.append(str(args.learning_rate))
    des.append(str(args.pre))
    des.append(str(args.batch_size))
    args.save_dir = os.path.join(args.base_dir,"_".join(des))
    #当前实验的模型是dsbw,w2a, 白化方法是INIternorm, group_size=64, 白化位置是all,迭代次数是10000,pretrain，lr=0.006,t=5
    args.m = "当前的数据集是{},实验模型是{}，白化方法是{},group_size是{},白化位置是{},迭代次数是{},源域是{},目标域是{},优化器是{},学习率为{},batch_size为{},{}".format(str(args.exp_setting),str(args.model_name),str(args.whitening),str(args.group_size),str(args.whitening_pos),str(args.iter),str(args.source_datasets[0]),str(args.target_datasets[0]),str(args.optimizer),str(args.learning_rate),str(args.batch_size),str(args.pre))
    ...
    # logger.info("run dataloader end")
    ...
    # logger.info("run enumerate dataloaders end")
    #****************************************************************************************
    ...
    #                                               Model Loading                                                     #
    ###################################################################################################################
    model = get_model(args.model_name, args.num_classes, args.in_features, num_domains=num_domains, pretrained=True, whitening=args.whitening, group_size=args.group_size, iter=args.iter, whitening_pos = args.whitening_pos, pre=args.pre, predir=args.predir)#
    ...
    for i_iter in range(start_iter, args.early_stop_step + 1):
        ...
        if args.dsbn:
            ...
            for src_idx, (x_s, y_s) in enumerate(src_inputs):
                pred_s, f_s = model(x_s, 0 * torch.ones(x_s.shape[0], dtype=torch.long).cuda(args.gpu),
                                    with_ft=True)
                src_preds.append((pred_s, f_s))

            trg_preds = []
            for trg_idx, (x_t, _) in enumerate(trg_inputs, num_source_domains):
                pred_t, f_t = model(x_t, 1 * torch.ones(x_t.shape[0], dtype=torch.long).cuda(args.gpu),
                                    with_ft=True)
        ...
        if args.dsbn:
            with torch.no_grad():
                model.eval()
                for trg_idx, (x_t, _) in enumerate(trg_inputs, num_source_domains):
                    pred_t_pseudo = model(x_t, 1 * torch.ones(x_t.shape[0], dtype=torch.long).cuda(args.gpu),
                                          with_ft=False)
        ...
        if i_iter % args.save_interval == 0 and i_iter != 0:
            ...
            for trg_idx, target_val_dataloader_iter in enumerate(target_val_dataloader_iters, num_source_domains):
                ...
                if args.use_tfboard:
                    writer.add_scalar('Accuracy', total_val_accuracy, i_iter)
                    writer.add_scalar('mean Accuracy', mean_val_accuracy, i_iter)
                    for c_name, c_val_acc in val_accuracy_each_c:
            ...
            if val_accuracy > best_accuracy:
                ...
                for d_idx in range(num_target_domains):
                    ...
                    contents.extend(["{}2{} iter:{}\n".format(args.source_dataset, args.target_datasets[d_idx], i_iter),
                ...
                best_result_path = os.path.join(args.save_dir, '{}_{}_best_result.txt'.format(
                    os.path.splitext(os.path.basename(__file__))[0], args.model_name))
